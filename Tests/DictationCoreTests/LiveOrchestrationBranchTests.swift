...
// дополнительно проверяется поведенчески напрямую в DictationCore
...
final class LiveOrchestrationBranchTests: XCTestCase {
    ...
    /// Сегменты и «хвост» (subscribeLiveDictation) идут в ТУ ЖЕ серийную
    ...
    @objc func testSegmentsAndFinalize_ShareSerialExecutor() {
        ...
        let subBody = Self.functionBody(named: "subscribeLiveDictation", in: source)
        XCTAssertTrue(
    ...
    /// Поведенческая фиксация (DictationCore доступен напрямую): recognizeSegment
