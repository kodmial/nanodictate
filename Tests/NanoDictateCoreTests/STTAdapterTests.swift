...
final class STTAdapterTests: XCTestCase {
    ...
    @objc func testGigaAMPlanEndToEnd() {
        // Live GigaAM: unknown id → openAICompatible multipart, language
        // forwarded, no extra auth steps. Verbose-поля (response_format /
        // timestamp_granularities[]) НЕ запрашиваются: произвольный сторонний
        // endpoint не гарантирует ни verbose_json, ни word-таймстампы.
        ...
        case .multipart(let data, let contentType):
            ...
            XCTAssertFalse(text.contains("response_format"),
                           "openAICompatible не запрашивает verbose_json — supportsVerboseJSON=false")
            XCTAssertFalse(text.contains("timestamp_granularities"),
                           "openAICompatible не просит word-таймстампы — supportsWordTimestamps=false")
        case .rawAudio:
