...
final class SilenceAutoStopTests: XCTestCase {
    ...
    /// Рубильник NANODICTATE_AUTOSTOP_DISABLED: 1/true выключают фичу,
    /// остальные ключи не влияют на сам рубильник.
    @objc func testFromEnvironmentDisabledSwitch() {
        for value in ["1", "true", "TRUE"] {
            let config = AutoStopConfig.fromEnvironment(["NANODICTATE_AUTOSTOP_DISABLED": value])
            XCTAssertFalse(config.enabled, "NANODICTATE_AUTOSTOP_DISABLED=\(value) выключает фичу")
        }
        ...
        let config = AutoStopConfig.fromEnvironment(["NANODICTATE_AUTOSTOP_DISABLED": "maybe"])
        XCTAssertTrue(config.enabled, "некорректное значение рубильника игнорируется")
    }

    /// Настройка длительности: NANODICTATE_AUTOSTOP_DURATION в секундах (Double).
    @objc func testFromEnvironmentDurationOverride() {
        let config = AutoStopConfig.fromEnvironment(["NANODICTATE_AUTOSTOP_DURATION": "5"])
    ...
    /// Настройка порога: NANODICTATE_AUTOSTOP_RMS в линейной шкале (Float).
    @objc func testFromEnvironmentRMSOverride() {
        let config = AutoStopConfig.fromEnvironment(["NANODICTATE_AUTOSTOP_RMS": "0.01"])
    ...
    @objc func testFromEnvironmentInvalidValuesIgnored() {
        let config = AutoStopConfig.fromEnvironment([
            "NANODICTATE_AUTOSTOP_DURATION": "abc",
            "NANODICTATE_AUTOSTOP_DURATION_EXTRA": "5", // другое имя не читается
            "NANODICTATE_AUTOSTOP_RMS": "-0.5", // порог должен быть > 0
        ])
    ...
    @objc func testFromEnvironmentCombined() {
        let config = AutoStopConfig.fromEnvironment([
            "NANODICTATE_AUTOSTOP_DURATION": "2",
            "NANODICTATE_AUTOSTOP_RMS": "0.005",
            "NANODICTATE_AUTOSTOP_DISABLED": "1",
        ])
    ...
    @objc func testConfigFieldsReachDetector() {
        let config = AutoStopConfig.fromEnvironment([
            "NANODICTATE_AUTOSTOP_DURATION": "1",
            "NANODICTATE_AUTOSTOP_RMS": "0.01",
        ])
        var detector = SilenceAutoStopDetector(
