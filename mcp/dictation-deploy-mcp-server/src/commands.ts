...
import {
  ...
  BUILD_TIMEOUT_MS,
  COMMAND_TIMEOUT_MS,
  DICTATORCTL_BUNDLE_ID,
  ENTITLEMENTS_AGENT_FILE,
  ENTITLEMENTS_DICTATORCTL_FILE,
  OUTPUT_TAIL_LINES,
  PROJECT_ROOT,
  SIGNING_IDENTITY,
  type Configuration,
...
const DICTATORCTL_SYMLINK = "/usr/local/bin/dictatorctl";
 ...
 */
export async function createDictatorctlSymlink(
  configuration: Configuration,
): Promise<SymlinkResult> {
  const target = binaryPaths(configuration).dictatorctl;
  const linkPath = DICTATORCTL_SYMLINK;
...
export async function buildProject(
  ...
  if (!toolchain.ok) {
    return {
      ...
      symlink: {
        created: false,
        target: paths.dictatorctl,
        linkPath: DICTATORCTL_SYMLINK,
  ...
  const symlink =
    buildOk
      ? await createDictatorctlSymlink(configuration)
      : {
          created: false,
          target: paths.dictatorctl,
          linkPath: DICTATORCTL_SYMLINK,
...
/** Sign both DictatorAgent and dictatorctl with the stable identity. */
export async function signProject(
  ...
  const ctlEntitlements = resolveEntitlementsPath(ENTITLEMENTS_DICTATORCTL_FILE);
...
/** Restart the LaunchAgent via `launchctl kickstart -k`. Fallback to `dictatorctl start`. */
export async function restartAgent(): Promise<RestartResult> {
  ...
  // Fallback: dictatorctl start (idempotent: returns "already running" if loaded)
  const dictctlPath = existsSync("/usr/local/bin/dictatorctl")
    ? "/usr/local/bin/dictatorctl"
    : "dictatorctl";
...
export async function getStatus(
  ...
  // 1. Process check
  const pgrep = await run("/usr/bin/pgrep", ["-f", "DictatorAgent"]);
  const pids =
    pgrep.status === 0
      ? pgrep.stdout
          .trim()
