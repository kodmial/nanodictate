/**
 * MCP tool definitions for the dictation-deploy-mcp-server.
 *
...
export function registerTools(server: McpServer): void {
  ...
  server.registerTool(
    "dictation_sign",
    {
      ...
      description:
        "Signs .build/<configuration>/{DictatorAgent,dictatorctl} with `codesign --force --sign \"" +
        SIGNING_IDENTITY +
  ...
  server.registerTool(
    "dictation_deploy",
    {
      ...
      description:
        "Full workflow in one call: swift build → codesign both binaries with the stable identity → restart the LaunchAgent (launchctl kickstart -k gui/<uid>/com.dictation.agent). Each step runs only if the previous one succeeded, so a failed build never restarts the running agent with a broken binary." +
        SHARED_FOOTER,
  ...
  server.registerTool(
    "dictation_restart",
    {
      ...
      description:
        "Restarts the com.dictation.agent LaunchAgent via `launchctl kickstart -k gui/<uid>/com.dictation.agent` (the same mechanism `dictatorctl`'s internal restart uses; there is no `dictatorctl restart` command). Falls back to `dictatorctl start` if kickstart fails (e.g. the agent was never loaded). Note: macOS does not reload the code signature from disk on kickstart — run dictation_sign before restarting for TCC-preserving rebuilds.",
  ...
  server.registerTool(
    "dictation_status",
    {
      ...
      description:
        "Reports: process state (pgrep -f DictatorAgent), LaunchAgent load state (launchctl print gui/<uid>/com.dictation.agent), and the code signature of .build/<configuration>/DictatorAgent (codesign -dv: signing identity, team id, entitlements). signatureStable = true means the identity matches \"" +
        SIGNING_IDENTITY +
