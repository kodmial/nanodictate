...
test("SymlinkSchema contract is enforced through BuildOutputSchema", () => {
  const ok = BuildOutputSchema.safeParse({
    ...
    configuration: "debug",
    exitCode: 0,
    binaryPath: "/p/.build/debug/DictatorAgent",
    symlink: { created: true, target: "/p/bin/dictatorctl", linkPath: "/usr/local/bin/dictatorctl", detail: "ok" },
    tail: "Build complete",
  });
...
test("SignOutputSchema parses the sign result contract", () => {
  ...
  assert.deepEqual(
    SignOutputSchema.parse({
      ...
      identity: "Dictation Code Signing",
      binaries: [bin],
...
test("CodeSignatureSchema and StatusOutputSchema parse the status contract", () => {
  const sig = {
    ...
    entitlementsFile: "/p/Resources/com.dictation.agent.entitlements",
  };
...
test("buildMarkdown renders success and failure", () => {
  const ok = buildMarkdown({
    ...
    exitCode: 0,
    binaryPath: "/p/DictatorAgent",
    symlink: { created: true, target: "/p/dictatorctl", linkPath: "/usr/local/bin/dictatorctl", detail: "symlink created" },
    tail: "Build complete",
  });
  assert.match(ok, /# Dictation build/);
  assert.match(ok, /configuration: `debug`/);
  assert.match(ok, /status: \*\*OK\*\* \(exit 0\)/);
  assert.match(ok, /binary: `\/p\/DictatorAgent`/);
  ...
  const fail = buildMarkdown({
    ...
    exitCode: 1,
    binaryPath: "/p/DictatorAgent",
    symlink: { created: false, target: "/p/dictatorctl", linkPath: "/usr/local/bin/dictatorctl", detail: "build failed — symlink not created" },
    tail: "error: boom",
  });
  assert.match(fail, /status: \*\*FAILED\*\* \(exit 1\)/);
  assert.match(fail, /binary: `\/p\/DictatorAgent` \(not created\)/);
...
test("signMarkdown renders success, partial, and failure with detail", () => {
  const ok = signMarkdown({
    ...
    identity: "Dictation Code Signing",
    binaries: [
      { name: "DictatorAgent", path: "/p/a", signed: true, verified: true, detail: "signed and verified" },
      { name: "dictatorctl", path: "/p/c", signed: true, verified: true, detail: "signed and verified" },
    ],
    detail: "All binaries signed and verified",
  });
  assert.match(ok, /# Dictation sign/);
  assert.match(ok, /overall: \*\*OK\*\*/);
  assert.match(ok, /DictatorAgent: \*\*signed\*\*, verified/);
  ...
  const partial = signMarkdown({
    ...
    identity: "Dictation Code Signing",
    binaries: [
      { name: "DictatorAgent", path: "/p/a", signed: true, verified: false, detail: "signed but verify returned 1: nope" },
      { name: "dictatorctl", path: "/p/c", signed: false, verified: false, detail: "binary not found" },
    ],
...
test("restartMarkdown renders success and failure", () => {
  const ok = restartMarkdown({
    success: true,
    method: "launchctl kickstart -k",
    detail: "Agent restarted via gui/501/com.dictation.agent",
  });
  assert.match(ok, /# Dictation agent restart/);
  assert.match(ok, /status: \*\*OK\*\*/);
  assert.match(ok, /method: `launchctl kickstart -k`/);

  const fail = restartMarkdown({
    success: false,
    method: "dictatorctl start (fallback)",
...
test("statusMarkdown renders running-with-signature and not-built branches", () => {
  ...
  assert.match(running, /# Dictation agent status/);
  ...
  assert.match(running, /stable signature \(identity = "Dictation Code Signing"\): \*\*yes\*\* — TCC grants preserved\*\*/);
...
test("deployMarkdown renders success, binary lines, and gated failures", () => {
  const ok = deployMarkdown({
    ...
    sign: {
      ...
      identity: "Dictation Code Signing",
      binaries: [
        { name: "DictatorAgent", signed: true, verified: true },
        { name: "dictatorctl", signed: true, verified: false },
      ],
    },
    restart: { success: true, method: "launchctl kickstart -k" },
    detail: "build → sign → restart OK",
  });
  assert.match(ok, /# Dictation deploy/);
  assert.match(ok, /overall: \*\*OK\*\*/);
  assert.match(ok, /## Build/);
  assert.match(ok, /\*\*OK\*\* \(exit 0\), binary `\/p\/DictatorAgent`, symlink created/);
  assert.match(ok, /DictatorAgent: signed, verified/);
  assert.match(ok, /dictatorctl: signed/);
  assert.match(ok, /\*\*OK\*\* via `launchctl kickstart -k`/);

  const failed = deployMarkdown({
    configuration: "release",
    success: false,
    build: { success: false, exitCode: 1, binaryPath: "/p/DictatorAgent", tail: "", symlink: { created: false } },
    sign: null,
    restart: null,
    detail: "build failed",
  });
  ...
  const signFailed = deployMarkdown({
    ...
    sign: {
      success: false,
      identity: "Dictation Code Signing",
      binaries: [{ name: "DictatorAgent", signed: false, verified: false }],
      detail: "codesign failed (exit 1): identity not found",
    },
    restart: null,
    detail: "sign failed",
  });
  assert.match(signFailed, /- \*\*FAILED\*\* — identity `Dictation Code Signing`/);
  assert.match(signFailed, /DictatorAgent: NOT signed/);
...
test("handleBuild renders markdown for a failing build", async () => {
  ...
  try {
    ...
    assert.match(res.content[0].text, /# Dictation build/);
...
test("handleStatus returns a shaped result from read-only probes", async () => {
  ...
  assert.match(res.content[0].text, /# Dictation agent status/);
