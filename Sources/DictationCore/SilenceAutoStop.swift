...
public struct AutoStopConfig: Equatable {
    ...
    /// (тот же путь, что `DICTATION_API_KEY` в `Config.swift`/`RetryProvider`).
    ///
    ...
    ///   • `DICTATION_AUTOSTOP_DISABLED=1` (или `true`) — рубильник: фича
    ///     выключена вовсе, любая пауза не останавливает запись;
    ///   • `DICTATION_AUTOSTOP_DURATION=5` — порог непрерывной тишины в
    ///     секундах (Double, строго > 0);
    ///   • `DICTATION_AUTOSTOP_RMS=0.01` — порог «тишины» одного буфера в
    ...
    public static func fromEnvironment(
        ...
        if env["DICTATION_AUTOSTOP_DISABLED"].map(parseDisabledFlag) ?? false {
            config.enabled = false
        }
        if let raw = env["DICTATION_AUTOSTOP_DURATION"], let d = Double(raw), d > 0 {
            config.requiredSilenceDuration = d
        }
        if let raw = env["DICTATION_AUTOSTOP_RMS"], let f = Float(raw), f > 0 {
            config.silenceRMSThreshold = f
        }
        return config
    }

    /// Признак «фича выключена» для `DICTATION_AUTOSTOP_DISABLED`.
