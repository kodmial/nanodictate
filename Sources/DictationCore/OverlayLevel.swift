...
public enum OverlayLevel {
    ...
    public static func enveloped(current: Float, target: Float, dt: TimeInterval) -> Float {
        let current = min(max(current, 0), 1)
        let target = min(max(target, 0), 1)
        if target > current {
            let step = Float(dt / attackTime)
            return min(target, current + step * (target - current))
        }
        let decayed = current * Float(exp(-dt / releaseTime))
...
public struct OverlayPeak: Equatable {
    ...
    public mutating func update(level: Float, dt: TimeInterval) -> Float {
        ...
        if level > value {
            ...
            value = max(0, value - Self.fallRate * Float(dt))
        }
        return value
    }
