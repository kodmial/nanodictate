...
func cmdStop() -> Int32 {
  ...
  // Legacy `launchctl unload` takes a plist PATH, not a service target
  // (bootout above already accepts the gui-domain target).
  let unload = runProcess("/bin/launchctl", ["unload", AgentService.canonicalURL().path])
  if unload.status == 0 {
    print("NanoDictate agent stopped")
    return 0
  }
...
func cmdTranscribeBatch(_ file: String, options: BatchTranscribeOptions) -> Int32 {
  ...
  if let out = options.outPath {
    ...
    // maxSegment in the key is also encoded in milliseconds — Int(30.5) and
    // Int(30) both truncated to 30, but 30500 and 30000 differ, so fractional
    // second values get distinct checkpoint identities (same policy as
    // overlap above). Old pre-fix temp checkpoints keyed "-30-" no longer
    // match the new key and are correctly ignored by resume.
    let maxSegmentKey = Int((options.maxSegment * 1000).rounded())
    let checkpointName =
      "nanodictate-batch-\(stableCheckpointStamp(inputURL.path))-\(provider.id)-"
      + "\(maxSegmentKey)-\(overlapKey)\(pauseFactor).checkpoint.json"
...
/// The probe must not create the file: `load` auto-copies the canon to
/// defaultPath when config.toml is missing, which would make `config init`
/// report "Config already exists" on a fresh machine.
let uiLanguage: String? = FileManager.default.fileExists(atPath: AppConfig.defaultPath())
  ? (try? AppConfig.load(from: nil))?.uiLanguage : nil
