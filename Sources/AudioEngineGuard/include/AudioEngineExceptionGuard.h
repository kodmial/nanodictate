...
// crash-репортом DictatorAgent). Этот модуль переводит исключение в NSError,
...
/// (домен "Domain.Dictation.AudioEngine", код 1).
FOUNDATION_EXPORT NSError * _Nullable DictationRunAudioEngineBlockGuarded(void (^ _Nonnull block)(void));

/// Тестовый триггер: поднимает NSException, чтобы unit-тест проверил
/// превращение исключения в ошибку (симуляция AVFAudio-краша).
FOUNDATION_EXPORT void DictationRaiseAudioEngineTestException(void);
