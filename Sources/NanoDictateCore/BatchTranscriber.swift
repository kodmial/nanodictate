...
// Файл 920 строк: пакетный пайплайн (планировщик + последовательный проход +
// параллельный пул + чекпоинт/resume + построение запроса) как один enum с
// чистыми функциями. Вынос части в отдельный файл/тип изменит public-контракт
// и доступ к private-состоянию — структурное правило отключено точечно.
...
public final class URLSessionBatchTransport: BatchHTTPTransport {
  ...
  public func send(request: URLRequest) async throws -> BatchHTTPResponse {
    ...
    for (key, value) in httpResponse.allHeaderFields {
      ...
      headers[stringKey.lowercased()] = stringValue
    }
...
private final class BatchRunState: @unchecked Sendable {
  ...
  init(total: Int, seeded: [BatchSegmentRecord?]) {
    self.total = total
    slots = Array(repeating: nil, count: total)
    ...
    while cursor < total, slots[cursor] != nil {
      cursor += 1
    }
    savedCount = cursor
  }
...
final class CheckpointWriter: @unchecked Sendable {
  ...
  func saveIfLonger(
    ...
    savedEnd = end
    let checkpoint = makeCheckpoint(end)
    try? saveCheckpoint(checkpoint, path)
  }
}
...
public enum BatchTranscriber {
  ...
  private static func execute(
    ...
    var slots = [BatchSegmentRecord?](repeating: nil, count: specs.count)
    if resume, let path = checkpointPath, let checkpoint = loadCheckpoint(path) {
      // swiftformat:disable indent wrapMultilineStatementBraces
      // (условия-продолжения: swiftformat переносит `{` на отдельную строку,
      // SwiftLint Kodeco этого запрещает — строка выравнивания под SwiftLint)
      ...
      // swiftformat:enable indent wrapMultilineStatementBraces
    }
...
public enum BatchRequestBuilder {
  ...
  /// сам решает формат тела (OpenAI-совместимый мультипарт, cloudflare —
