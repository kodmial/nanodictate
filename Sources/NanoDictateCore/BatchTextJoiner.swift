...
public enum BatchTextJoiner {
    ...
    public static func boundaryDropCount(previous: String, next: String) -> Int {
        ...
        // Наибольшее k, где k последних слов previous == k первых слов next.
        for k in stride(from: maxK, through: 1, by: -1) {
            let prevSuffix = prevWords.suffix(k).map { $0.lowercased() }
            let nextPrefix = nextWords.prefix(k).map { $0.lowercased() }
            if prevSuffix == nextPrefix { return k }
        }
        return 0
    }
