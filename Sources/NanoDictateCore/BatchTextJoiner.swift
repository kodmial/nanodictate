...
public enum BatchTextJoiner {
  ...
  public static func join(_ texts: [String]) -> String {
    ...
    for raw in texts {
      ...
      // Placeholder (skipped chunk) is joined verbatim on both sides: it
      // never participates in boundary dedup, so consecutive skipped chunks
      // keep their own "[…]" markers.
      let drop =
        previous == BatchTranscriber.placeholder || text == BatchTranscriber.placeholder
        ? 0 : boundaryDropCount(previous: previous, next: text)
