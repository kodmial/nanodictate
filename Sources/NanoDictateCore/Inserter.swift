...
/// Полный слепок буфера обмена: все items пастеборда со всеми их типами и
/// данными. Restore по такому слепку восстанавливает и не-text контент
/// (изображения, файлы и т.д.), а не только строку.
public typealias ClipboardSnapshot = [[(NSPasteboard.PasteboardType, Data)]]
...
public struct ClipboardInsertBridge {
  /// Current pasteboard snapshot ([] = empty clipboard).
  public var readClipboard: () -> ClipboardSnapshot
  /// Write snapshot to pasteboard ([] = clear contents).
  public var writeClipboard: (ClipboardSnapshot) -> Void
  /// Emulate Cmd+V.
  ...
  public init(
    readClipboard: @escaping () -> ClipboardSnapshot =
      ClipboardInsertBridge.defaultReadClipboard,
    writeClipboard: @escaping (ClipboardSnapshot) -> Void =
      ClipboardInsertBridge.defaultWriteClipboard,
  ...
  /// Snapshot из одного строкового item (текст диктовки для Cmd+V);
  /// пустая строка — пустой слепок (пастеборд будет очищен).
  static func snapshot(containingText text: String) -> ClipboardSnapshot {
    guard !text.isEmpty else { return [] }
    return [[(NSPasteboard.PasteboardType.string, Data(text.utf8))]]
  }

  public static func defaultReadClipboard() -> ClipboardSnapshot {
    guard let items = NSPasteboard.general.pasteboardItems else { return [] }
    // Данные копируются сразу (до clearContents): после очистки пастеборда
    // NSPasteboardItem свои data(forType:) уже не отдаёт, а восстановление
    // должно быть полным — все items и типы (не только .string).
  ...
  public static func defaultWriteClipboard(_ snapshot: ClipboardSnapshot) {
    let pasteboard = NSPasteboard.general
    pasteboard.clearContents()
    guard !snapshot.isEmpty else { return }
    let items = snapshot.map { typeData in
