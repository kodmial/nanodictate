...
public struct AutoStopConfig: Equatable {
    ...
    public static func fromEnvironment(
        ...
        if let raw = env["NANODICTATE_AUTOSTOP_DURATION"], let d = Double(raw), d.isFinite, d > 0 {
            config.requiredSilenceDuration = d
        }
        if let raw = env["NANODICTATE_AUTOSTOP_RMS"], let f = Float(raw), f.isFinite, f > 0 {
            config.silenceRMSThreshold = f
        }
        if let raw = env["NANODICTATE_AUTOSTOP_SPEECH_RMS"], let f = Float(raw), f.isFinite, f > 0 {
            config.speechRMSThreshold = f
        }
