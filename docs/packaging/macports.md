...
`$(port prefix)/share/nanodictate/`. Unlike Homebrew, the port has no
post-install step: `destroot` only stages the binaries and the config, never
touches a user's home directory, and MacPorts has no mechanism for
user-scope LaunchAgents (a `startupitem` would create a second, system-level
daemon). So the install does **not** register the agent — you start it once
yourself with `nanodictate start` (step 3 below), after which RunAtLoad
keeps it running at login.
...
3. **Register and start the agent.** `nanodictate start` writes the
   canonical plist `~/Library/LaunchAgents/com.nanodictate.agent.plist`
   (label `com.nanodictate.agent`, ProgramArguments = the
   `$(port prefix)/bin/NanoDictateAgent` binary, RunAtLoad + KeepAlive) and
   bootstraps it into your gui domain — the same single label and file the
   Homebrew post_install uses. Then manage it with:
