# Дизайн: NanoDictate — macOS-диктовка по двойному Alt
...
SwiftPM-мультитаргет: библиотека `NanoDictateCore` + 2 исполняемых таргета
(`NanoDictateAgent` — фоновый агент, `nanodictate` — CLI).
...
### 8. Config
- `~/.config/nanodictate/config.toml` (или рядом с бинарём).
...
### 10. LaunchAgent
- plist в `~/Library/LaunchAgents/com.nanodictate.agent.plist`
  (Write via `launchctl bootstrap`), `RunAtLoad = true`, `KeepAlive`.
- Логи в `~/Library/Logs/NanoDictate/agent.log` (и stderr/stdout туда).
...
- Сеть: 1 ретрай, затем окно «Не удалось распознать: <причина>»; запись
  отбрасывается.
- Логи: файл в `~/Library/Logs/NanoDictate/`.
...
Интеграция (ручная):
- `nanodictate transcribe example/Новая запись 2.m4a` — сверка с фактическим
